...
def dictionary_of_metrics(items):
  ...
  new_dict['median'] = round(np.median(items),2)
  new_dict['var'] = round(np.var(items,ddof=1), 2)
  new_dict['std'] = round(np.sqrt(np.var(items,ddof=1)),2)
  new_dict['min'] = round(np.min(items),2)
  new_dict['max'] = round(np.max(items),2)
...
def word_splitter(df):
  ...
  """
  #split the tweets column and lowercase the words
